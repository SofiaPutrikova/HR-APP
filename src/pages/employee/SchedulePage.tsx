...
export function EmployeeSchedulePage() {
  ...
  const weekLabel = (() => {
    ...
    return `${days[0].toLocaleDateString('ru-RU', opts)} – ${days[6].toLocaleDateString('ru-RU', opts)}`
  })()
  ...
  return (
    <div className="p-8">
      <div className="mb-6">
        <h2 className="text-2xl font-bold">Расписание сотрудников</h2>
        <p className="text-muted-foreground mt-1">Рабочие графики на неделю</p>
      </div>
      ...
      <Card>
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <p className="p-6 text-sm text-muted-foreground">Сотрудники не найдены</p>
          ) : (
                          ...
                          <td key={dateStr} className={`px-2 py-3 text-center ${isToday ? 'bg-primary/5' : ''}`}>
                            {schedule ? (
                              <div className="inline-flex flex-col items-center gap-0.5">
                                <Badge
                                  variant="secondary"
                                  className="text-xs font-normal px-2 py-0.5 whitespace-nowrap"
                                >
